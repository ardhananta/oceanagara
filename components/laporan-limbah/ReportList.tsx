...
export default function ReportList({ reports, filter, onFilter, onSelect }: ReportListProps) {
      ...
      <div className="flex flex-wrap gap-2">
        {(['semua', 'verified', 'suspected', 'rejected'] as WasteFilter[]).map((f) => (
          <button
            key={f}
            onClick={() => onFilter(f)}
            className={`px-3.5 py-1.5 rounded-full border text-[10px] font-extrabold uppercase tracking-wider transition-all duration-200 ${
              filter === f
                ? 'bg-[#162e52] text-white border-[#162e52] shadow-md shadow-[#162e52]/20 scale-[1.02]'
                : 'bg-zinc-50 text-zinc-600 border-zinc-200 hover:border-[#162e52]/40 hover:bg-white'
            }`}
          >
            {f === 'semua' ? 'Semua' : STATUS_META[f].label} · {counts[f]}
          </button>
        ))}
      </div>

      {reports.length === 0 ? (
        <div className="text-center py-12 text-zinc-400 bg-zinc-50/60 rounded-2xl border border-dashed border-zinc-200">
          ...
          <p className="text-xs font-bold text-zinc-600">Belum ada laporan warga.</p>
          <p className="text-[10px] text-zinc-400 mt-0.5">Laporan akan otomatis muncul setelah dikirimkan dari dashboard masyarakat.</p>
        </div>
      ) : (
          ...
          {reports.map((r) => {
            ...
            return (
              <li
                key={r.id}
                onClick={() => onSelect(r)}
                className="group flex items-center gap-3.5 p-3.5 rounded-xl border border-zinc-200 bg-white hover:bg-sky-50/50 hover:border-sky-300/90 shadow-sm hover:shadow-md transition-all duration-200 cursor-pointer"
              >
                {r.photoThumbs[0] ? (
                  <img
                    src={r.photoThumbs[0]}
                    alt="Foto laporan"
                    className="w-14 h-14 rounded-xl object-cover border border-zinc-200 flex-shrink-0 group-hover:scale-105 transition-transform"
                  />
                ) : (
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs font-extrabold text-[#162e52] truncate mt-1.5 capitalize group-hover:text-sky-900 transition-colors">
                  ...
                  <div className="flex items-center gap-2 mt-1 text-[9px] text-zinc-500 font-medium">
                    <span className="font-mono font-bold text-zinc-600">{reportDisplayCode(r.id)}</span>
                    <span>·</span>
