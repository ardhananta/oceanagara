    ...
    const unsubscribe = onAuthChange(async (user) => {
      if (!user) {
        ...
        if (uProfile && uProfile.role === 'nelayan') {
          ...
          // Wrong role for this dashboard, redirect based on user role
          const paths = {
      ...
      <main className="flex-grow p-6 md:p-12 max-w-5xl w-full mx-auto flex flex-col justify-center">
        <div className="border border-zinc-800 p-8 md:p-12 bg-zinc-900/50 backdrop-blur-sm relative">
          ...
          <h1 className="text-2xl md:text-3xl font-extrabold uppercase tracking-tight mb-4 text-white">
            Selamat Datang, {profile?.displayName || 'Nelayan'}!
          </h1>
          <p className="text-sm text-zinc-400 leading-relaxed mb-8 max-w-xl">
            Anda masuk ke Dashboard khusus Nelayan Tradisional. Gunakan menu navigasi untuk memantau data cuaca harian, log tangkapan tradisional, dan peta batas aman berlayar.
          </p>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-6 border-t border-zinc-800">
            <div>
              <span className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest block mb-1">Nama Kapal</span>
              <p className="text-sm font-semibold text-white uppercase">{String(profile?.namaKapal || '-')}</p>
            </div>
            <div>
              <span className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest block mb-1">Wilayah Operasi</span>
              <p className="text-sm font-semibold text-white uppercase">{String(profile?.wilayahOperasi || '-')}</p>
            </div>
          </div>
        </div>
      </main>
